...
voting_model = VotingClassifier(estimators=[('SVC(linear)', models[3]), ('Kn1', models[1]), ('SVC(poly)', models[4])],
                                voting='hard')
voting_model.fit(X_train, y_train)
voting_score = voting_model.score(X_test, y_test)  # Model score
print(f'\nVoting Classifier with hard voting has score {voting_score * 100}%')
voting_score5, std5 = lib.evaluate_classifier(voting_model, X_train, y_train)  # Model 5-fold CV
...
score_nb = bilbo.score(X_test, y_test)  # Model score
print(f'\nBagging Classifier (Custom NB) has score {score_nb * 100}%')
score_nb5, std_nb5 = lib.evaluate_classifier(bilbo, X_train, y_train)  # Model 5-fold CV
...
score_svc = bilbo_svc.score(X_test, y_test)  # Model score
print(f'\nBagging Classifier (SVC poly) has score {score_svc * 100}%')
score_svc5, std_svc5 = lib.evaluate_classifier(bilbo_svc, X_train, y_train)  # Model 5-fold CV
